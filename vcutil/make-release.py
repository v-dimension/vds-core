...
def main(args=sys.argv[1:]):
    """
    Perform the final Vds release process up to the git tag.
    """
...
def build():
    ...
    sh_log('./vcutil/build.sh', '-j', nproc)
...
def gen_release_notes(release):
    sh_log('python', './vcutil/release-notes.py', '--version', release.novtext)
    sh_log(
        'git',
        'add',
        './doc/authors.md',
...
def update_debian_changelog(release):
    os.environ['DEBEMAIL'] = ''
    os.environ['DEBFULLNAME'] = 'Vds Company'
    sh_log(
        'debchange',
...
def initialize_logging():
    ...
    logging.info('vds make-release.py debug log: %r', logname)
