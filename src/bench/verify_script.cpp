...
static void VerifyScriptBench(benchmark::State& state)
{
    ...
    key.Sign(SignatureHash(witScriptPubkey, txSpend, 0, SIGHASH_ALL, SIGVERSION_WITNESS_V0), witness.stack.back(), 0);
    ...
    while (state.KeepRunning()) {
        ...
        bool success = VerifyScript(
                           ...
                           flags,
                           MutableTransactionSignatureChecker(&txSpend, 0),
                           &err);
        ...
        stream << txSpend;
        int csuccess = zcashconsensus_verify_script(
