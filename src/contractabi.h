...
#define paternBytes32 "^[a-fA-F0-9]{%d,%d}$"
...
public:
    ...
    static bool getRegularExpession(const ParameterType &paramType, boost::regex &regEx);
    ...
    std::string ToString() const  {
        return strprintf("%s %s", type, name);
    };
...
public:
    ...
    std::string ToString() const {
        ...
        return strprintf("%s %s(%s) public %s returns (%s) %s %s", type, name, inPuts, constant ? "const": "", outPuts, payable?"_payable": "", anonymous ? "anon" : "");
    };
...
public:
    ...
    std::string ToString() const {
        std::string outPut;
        for (const auto &AbiFunc: functions) {
            outPut += AbiFunc.ToString();
        }
        return outPut;
    };
};
