...
/**
 * here we add this class just for SaplingMerkleTree initilization.
 * a static function object should initialize only in a class.
 * @brief The Useless class
 */
class Useless
{
public:
...
static int AppInitRawTx(int argc, char* argv[])
{
    ...
    if (argc < 2 || mapArgs.count("-?") || mapArgs.count("-h") || mapArgs.count("-help")) {
        ...
        std::string strUsage = _("Vds vds-tx utility version") + " " + FormatFullVersion() + "\n\n" +
                               _("Usage:") + "\n" +
                               "  vds-tx [options] <hex-tx> [commands]  " + _("Update hex-encoded zcash transaction") + "\n" +
                               "  vds-tx [options] -create [commands]   " + _("Create hex-encoded zcash transaction") + "\n" +
                               "\n";
...
static void MutateTxAddOutAddr(CMutableTransaction& tx, const string& strInput)
{
    ...
    CTxOut txout(value, CTxOut::NORMAL, scriptPubKey);
...
static void MutateTxSign(CMutableTransaction& tx, const string& flagStr)
{
    ...
    for (unsigned int i = 0; i < mergedTx.vin.size(); i++) {
        ...
        if (!VerifyScript(txin.scriptSig, prevPubKey, nullptr, STANDARD_SCRIPT_VERIFY_FLAGS, TransactionSignatureChecker(&txConst, i)))
            fComplete = false;
    }
