...
#define LogPrintHex(category, data, size, ...) do { MarkUsed(__VA_ARGS__); } while(0)
#else
#define LogPrintf(...) do { \
...
#define LogPrint(category, ...) do { \
    if (LogAcceptCategory((category))) { \
        ...
        try { \
            _log_msg_ = tfm::format("[%s]: ", category) + tfm::format(__VA_ARGS__); \
        } catch (tinyformat::format_error &fmterr) { \
            /* Original format string will have newline so don't add one here */ \
            _log_msg_ = "Error \"" + std::string(fmterr.what()) + "\" while formatting log message: " + FormatStringFromLogArgs(__VA_ARGS__); \
        } \
...
#define LogPrintHex(category, data, size, ...) do { \
    if (LogAcceptCategory((category))) { \
        std::string _log_msg_; \
        try { \
            _log_msg_ = tfm::format("[%s]: ", category) + tfm::format(__VA_ARGS__); \
            _log_msg_ += tfm::format("\n"); \
            for (int i = 0; i < (size); i++) { \
                _log_msg_ += tfm::format("0x%02X, ", (data)[i]); \
            } \
            _log_msg_ += tfm::format("\n\n"); \
        } catch (tinyformat::format_error &fmterr) { \
            /* Original format string will have newline so don't add one here */ \
...
#define DebugOutf(...) LogPrintf("%s:%d(%s)  %s\n", __FILE__, __LINE__, __FUNCTION__, __VA_ARGS__)
#define DebugOut(category, ...) LogPrint(category, "%s:%d(%s)  %s\n", __FILE__, __LINE__, __FUNCTION__, __VA_ARGS__)
...
{
    std::string s = strprintf("vds-%s", name);
...
{
    std::ostringstream oss;  //create a format stream
    oss << t;           //add value to stream
    return oss.str();
}
