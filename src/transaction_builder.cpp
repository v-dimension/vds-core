// Copyright (c) 2018 The Vds developers
...
bool TransactionBuilder::AddTransparentOutput(CTxDestination& to, CAmount value)
{
    ...
    CTxOut out(value, CTxOut::NORMAL, scriptPubKey);
...
bool TransactionBuilder::IsSendChangeT()
{
    if(tChangeAddr){
        return true;
    }
...
bool TransactionBuilder::IsSendChangeZ()
{
    if(zChangeAddr){
        return true;
    }
...
boost::optional<CTransaction> TransactionBuilder::Build()
{
    ...
    CScript scriptCode;
    try {
        dataToBeSigned = SignatureHash(scriptCode, mtx, NOT_AN_INPUT, SIGHASH_ALL, SIGVERSION_BASE, nullptr);
    ...
    for (int nIn = 0; nIn < mtx.vin.size(); nIn++) {
        ...
        bool signSuccess = ProduceSignature(
                               TransactionSignatureCreator(
                                   keystore, &txNewConst, nIn, SIGHASH_ALL),
