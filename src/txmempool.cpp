...
void CTxMemPool::addSpentIndex(const CTxMemPoolEntry& entry, const CCoinsViewCache& view)
{
    ...
    for (unsigned int j = 0; j < tx.vin.size(); j++) {
        ...
        GetIndexKey(prevout.scriptPubKey, addressHash, addressType);
...
void CTxMemPool::removeUnchecked(txiter it, MemPoolRemovalReason reason)
{
    ...
    for (const SpendDescription& spendDescription : it->GetSharedTx()->vShieldedSpend) {
...
void CTxMemPool::removeWithAnchor(const uint256& invalidRoot, ShieldedType type)
{
    ...
    BOOST_FOREACH(const CTransaction & tx, transactionsToRemove) {
        ...
        removeRecursive(tx, MemPoolRemovalReason::REORG);
    }
}
...
void CTxMemPool::removeConflicts(const CTransaction& tx)
{
    ...
    if (tx.IsCoinClue()) {
        ...
        // check duplicated clue
        if (cluepool.IsConflict(dest, tx.GetHash())) {
            CClue clue;
            if (cluepool.GetClue(dest, clue)) {
                indexed_transaction_set::iterator i = mapTx.find(clue.txid);
                if (i != mapTx.end()) {
                    const CTransaction& txConflict = i->GetTx();
                    removeRecursive(txConflict, MemPoolRemovalReason::CONFLICT);
                    cluepool.EraseClue(dest);
                }
            }
        }
        ...
        for (auto& out : tx.vout) {
            if (out.nFlag == CTxOut::CLUE) {
                if (!ExtractDestination(out.scriptPubKey, parent))
                    return;
                if (cluepool.ChildrenSize(parent) > Params().ClueChildrenWidth()) {
                    if (!cluepool.GetChildren(parent, vChildren))
                        break;
                }
                break;
            }
...
void CTxMemPool::removeForBlock(const std::vector<CTransactionRef>& vtx, unsigned int nBlockHeight)
{
    ...
    for (const auto& tx : vtx) {
        ...
        if (it != mapTx.end()) {
            ...
            if (tx->IsCoinClue()) {
                cluepool.DeleteTxClue(tx->GetHash());
                int parentCount = 0;
                CTxDestination root;
                ...
                for (const CTxOut& out : tx->vout) {
                    ...
                    if (out.nFlag & CTxOut::CLUE) {
                        parentCount++;
                        if (ExtractDestination(out.scriptPubKey, root)) {
                            CClue clue;
                            if (cluepool.GetClue(root, clue)
                                    && IsNullTxDestination(clue.inviter)
                                    && IsNullTxDestination(clue.parent)
                                    && clue.txid.IsNull()) {
                                break;
                            }
                        }
...
static void CheckInputsAndUpdateCoins(const CTransaction& tx, CCoinsViewCache& mempoolDuplicate, const int64_t spendheight)
{
    ...
    CAmount txfee = 0;
    bool fCheckResult = tx.IsCoinBase() || Consensus::CheckTxInputs(tx, state, mempoolDuplicate, cluepool, spendheight, Params().GetConsensus(), txfee);
    assert(fCheckResult);
    if (tx.IsCoinClue())
        cluepool.DeleteTxClue(tx.GetHash());
...
void CTxMemPool::CheckBiggestBid(const int& nHeight)
{
    for (auto itBiggest = mapBiggestBid.begin(); itBiggest != mapBiggestBid.end();) {
        int nHeightBid = itBiggest->first;
        if (nHeightBid <= nHeight) {
            uint256 txHash = itBiggest->second.first;
            mapBiggestBid.erase(itBiggest++);
            {
                txiter it = mapTx.find(txHash);
                if (it != mapTx.end()) {
                    setEntries stage;
                    stage.insert(it);
                    RemoveStaged(stage, true, MemPoolRemovalReason::BLOCK);

                    CTransactionRef ptx = this->get(txHash);
                    if (ptx) {
                        removeConflicts(*ptx);
...
size_t CTxMemPool::DynamicMemoryUsage() const
{
    ...
    return memusage::MallocUsage(sizeof(CTxMemPoolEntry) + 12 * sizeof(void*)) * mapTx.size() + memusage::DynamicUsage(mapNextTx) + memusage::DynamicUsage(mapDeltas) + memusage::DynamicUsage(mapLinks) + memusage::DynamicUsage(vTxHashes) /*+ memusage::DynamicUsage(mapBiggestBid) + memusage::DynamicUsage(mapAddressInserted) + memusage::DynamicUsage(mapAddress) + memusage::DynamicUsage(mapSaplingNullifiers) */ + cachedInnerUsage;
}
...
bool CClueViewMemPool::BatchWrite(CClueMap& mapClue, SeasonRankMap& mapSeason, const uint256& hashBlockIn)
{
    return false;
}

bool CClueViewMemPool::AddClue(const CClue& clue)
{
    if (!CClueViewCache::AddClue(clue))
        return false;

    cacheTx.insert(std::make_pair(clue.txid, clue.address));
    return true;
}

bool CClueViewMemPool::AddRankItem(const CTxDestination& dest, int nSeason, const CRankItem& item)
{
    return false;
}

bool CClueViewMemPool::DeleteTxClue(const uint256& hash)
{
    if (cacheTx.find(hash) != cacheTx.end()) {
        if (CClueViewCache::EraseClue(cacheTx[hash]))
            cacheTx.erase(hash);
    }
    return true;
}

bool CClueViewMemPool::Flush()
{
    return false;
}
