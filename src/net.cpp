...
CNode* CConnman::ConnectNode(CAddress addrConnect, const char* pszDest)
{
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, Params().GetDefaultPort(), nConnectTimeout, &proxyConnectionFailed) :
        ...
        // Add node
        CNode* pnode = new CNode(GetNewNodeId(), nLocalServices, GetBestHeight(), 0, hSocket, addrConnect, pszDest ? pszDest : "", false, true);
...
void CNode::copyStats(CNodeStats& stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (vds users should be well used to small numbers with many decimal places by now :)
...
struct NodeEvictionCandidate {
    NodeEvictionCandidate(CNode* pnode)
        : id(pnode->id),
          ...
          fBloomFilter(pnode->pfilter != nullptr),
...
void CConnman::AcceptConnection(const ListenSocket& hListenSocket)
{
    ...
    CNode* pnode = new CNode(GetNewNodeId(), nLocalServices, GetBestHeight(), 0, hSocket, addr, "", true);
...
void ThreadMapPort()
{
    ...
    if (r == 1) {
        ...
        std::string strDesc = "Vds Core " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService& addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR) {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. vds Core is probably already running."), addrBind.ToString());
        else
...
CConnman::CConnman()
{
    ...
    EC_KEY_generate_key(privkey);
    if (!EC_KEY_check_key(privkey))
        return;
    unsigned char t[CPubKey::PUBLIC_KEY_SIZE];
    unsigned char* p = (unsigned char*)t;
    i2o_ECPublicKey(privkey, &p);
    std::vector<unsigned char> tmp((uint8_t*)t, (uint8_t*)(t + CPubKey::PUBLIC_KEY_SIZE));
    pubKey.Set(tmp.begin(), tmp.end());
}
...
bool CConnman::Start(CScheduler& scheduler, std::string& strNodeError, Options connOptions)
{
    ...
    if (pnodeLocalHost == NULL) {
        CNetAddr local;
        LookupHost("127.0.0.1", local, false);
        pnodeLocalHost = new CNode(GetNewNodeId(), nLocalServices, GetBestHeight(), 0, INVALID_SOCKET, CAddress(CService(local, 0), nLocalServices));
        pnodeLocalHost->AddRef();
...
bool CConnman::SignMessage(const string strMessage, std::vector<unsigned char>& vchSigRet)
{
    uint8_t dataOut[100];
    uint32_t lenOut = 0;

    uint256 dataHash = Hash(strMessage.begin(), strMessage.end());

    if (!ECDSA_sign(0, (uint8_t*)dataHash.begin(), dataHash.size(), dataOut, &lenOut, privkey)) {
        return false;
    }
    std::vector<unsigned char> dataSigOut((uint8_t*)dataOut, (uint8_t*)dataOut + lenOut);
    vchSigRet = dataSigOut;
    return true;
}

bool CConnman::VerifyMessage(const CPubKey pubkeyIn, const std::vector<unsigned char>& vchSig, const string strMessage)
{
    EC_KEY* pubEcKey = EC_KEY_new_by_curve_name(NID_secp256k1);

    uint8_t bufPub[100] = {0};
    memcpy((uint8_t*)bufPub, pubkeyIn.begin(), pubkeyIn.size());

    const unsigned char* p = (uint8_t*)bufPub;
    o2i_ECPublicKey(&pubEcKey, &p, pubkeyIn.size());

    uint8_t dataIn[100];
    memcpy((uint8_t*)dataIn, &vchSig[0], vchSig.size());

    uint256 dataHash = Hash(strMessage.begin(), strMessage.end());
    if (ECDSA_verify(0, (uint8_t*)dataHash.begin(), dataHash.size(), dataIn, vchSig.size(), pubEcKey) != 1) {
...
CNode::CNode(NodeId idIn, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, int nMyStartingLNHeightIn, SOCKET hSocketIn, const CAddress& addrIn, const std::string& addrNameIn, bool fInboundIn, bool fNetworkNodeIn) :
...
std::vector<unsigned char> CNode::CalculateKeyedNetGroup(CAddress& address)
{
    ...
    hash.Write(vchGroup.data(), vchGroup.size());

    hash.Write(vchSecretKey.data(), vchSecretKey.size());
