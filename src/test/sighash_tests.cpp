...
//            jsdesc.proof = libzcash::VCProof::random_invalid();
...
//        crypto_sign_keypair(tx.joinSplitPubKey.begin(), joinSplitPrivKey);
        ...
        uint256 dataToBeSigned = SignatureHash(scriptCode, signTx, NOT_AN_INPUT, SIGHASH_ALL, SIGVERSION_BASE);
...
BOOST_AUTO_TEST_CASE(sighash_test)
{
    ...
    for (int i = 0; i < nRandomTests; i++) {
        ...
        sh = SignatureHash(scriptCode, txTo, nIn, nHashType, SIGVERSION_BASE);
#if defined(PRINT_SIGHASH_JSON)
...
BOOST_AUTO_TEST_CASE(sighash_from_data)
{
    ...
    for (size_t idx = 0; idx < tests.size(); idx++) {
        ...
        sh = SignatureHash(scriptCode, tx, nIn, nHashType, SIGVERSION_BASE);
        BOOST_CHECK_EQUAL(sh.GetHex(), sigHashHex);
    }
}
