...
static void add_coin(const CAmount& nValue, int nAge = 6 * 24, bool fIsFromMe = false, int nInput = 0)
{
    ...
    CTransaction mtx(tx);
    CTransactionRef rtx = MakeTransactionRef(mtx);
    CWalletTx* wtx = new CWalletTx(&wallet, rtx);
    if (fIsFromMe) {
        wtx->fDebitCached = true;
        wtx->nDebitCached = 1;
    }
    COutput output(wtx, nInput, CTxOut::NORMAL, nAge, true);
...
BOOST_AUTO_TEST_CASE(coin_selection_tests, * boost::unit_test::disabled())
{
    ...
    for (int i = 0; i < RUN_TESTS; i++) {
        ...
        add_coin( 5 * CENT); // now we have 5+6+7+8+20+30 = 76 cents total
        ...
        add_coin( 18 * CENT); // now we have 5+6+7+8+18+20+30 = 94
...
BOOST_AUTO_TEST_CASE(ApproximateBestSubset, * boost::unit_test::disabled())
{
    CoinSet setCoinsRet;
    CAmount nValueRet;
