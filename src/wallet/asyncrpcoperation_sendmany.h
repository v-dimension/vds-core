...
#define ASYNC_RPC_OPERATION_DEFAULT_MINERS_FEE   500000
...
// Input UTXO is a tuple (quadruple) of txid, vout, amount, coinbase, flag)
typedef std::tuple<uint256, int, CAmount, bool, uint8_t> SendManyInputUTXO;
...
public:
    ...
    bool IsGuiMode();
    void SetGuiMode(bool _isGuiMode);

    bool HasSelected() const;
    bool HasSelected_tAddr() const;

    bool IsSelected(const SaplingNoteEntry& noteEntry) const;
    bool IsSelected(const SendManyInputUTXO &output) const;

    void Select(const SaplingNoteEntry& noteEntry);
    void Select(const SendManyInputUTXO& output);

    void UnSelect(const SaplingNoteEntry& noteEntry);
    void UnSelect(const SendManyInputUTXO& output);

    void UnSelectAll();
    void UnSelectAll_tAddr();

    void ListSelected(std::vector<SaplingNoteEntry>& vNoteEntrys) const;
    void ListSelected(std::vector<SendManyInputUTXO>& vOutpoints) const;

    bool testmode = false;  // Set to true to disable sending txs and generating proofs

    bool paymentDisclosureMode = false; // Set to true to save esk for encrypted notes in payment disclosure database.

    boost::signals2::signal<void (string strIdOperation, int nCodeError, string strErrorReason, const uint256& hashTx)> NotifyOperationStateUpdate;
...
private:
    ...
    std::vector<SendManyInputUTXO> t_vSelectedInputs;
    std::vector<SaplingNoteEntry> z_vSelectedInputs;
    ...
    bool find_unspent_notes_in_selected();
    bool find_utxos(bool fAcceptCoinbase);
    bool find_utxos_in_selected(bool fAcceptCoinbase);
