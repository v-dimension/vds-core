...
uint256 GetWitnessesAndAnchors(TestWallet& wallet,
...
TEST(WalletTests, GetConflictedSaplingNotes)
{
    ...
    auto maybe_pt = libzcash::SaplingNotePlaintext::decrypt(
                        wtx.tx->vShieldedOutput[0].encCiphertext,
                        ivk,
                        wtx.tx->vShieldedOutput[0].ephemeralKey,
                        wtx.tx->vShieldedOutput[0].cm);
...
TEST(WalletTests, SpentSaplingNoteIsFromMe)
{
    ...
    wtx.SetMerkleBranch(&fakeIndex2, 0);
    wallet.AddToWallet(wtx2, true, NULL);
...
//        index1.nHeight = 1;
//        auto outpts = CreateValidBlock(wallet, sk, index1, block1, saplingTree);
...
TEST(WalletTests, MarkAffectedSaplingTransactionsDirty)
{
    ...
    // 0.0005 t-VC in, 0.0004 z-VC out, 0.0001 t-VC fee
    ...
    // 0.0004 z-VC in, 0.00025 z-VC out, 0.0001 t-VC fee, 0.00005 z-VC change
    auto builder2 = TransactionBuilder(consensusParams, 2);
