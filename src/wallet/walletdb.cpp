...
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("keymeta"), vchPubKey), keyMeta);
}
...
bool CWalletDB::ReadScriptMeta(const CScriptID& scriptID, CKeyMetadata& metaData)
{
    return Read(std::make_pair(std::string("scriptmeta"), scriptID), metaData);
}

bool CWalletDB::WriteScriptMeta(const CScriptID& scriptID, const CKeyMetadata& metaData)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("scriptmeta"), scriptID), metaData, true);
}
...
bool CWalletDB::EraseScriptMeta(const CScriptID& scriptID)
{
    nWalletDBUpdated++;
    return Erase(std::make_pair(std::string("scriptmeta"), scriptID));
}
...
DBErrors CWalletDB::ReorderTransactions(CWallet* pwallet)
{
    ...
    for (TxItems::iterator it = txByTime.begin(); it != txByTime.end(); ++it) {
        ...
        int64_t unOrder = -1;
        int64_t& nOrderPos = (pwtx != 0) ? pwtx->nOrderPos : unOrder;
             ...
             CWalletScanState& wss, string& strType, string& strErr)
{
    try {
        ...
        if (strType == "name") {
        ...
        } else if (strType == "scriptmeta") {
            CScriptID scriptID;
            ssKey >> scriptID;
            CKeyMetadata meta;
            ssValue >> meta;
            pwallet->mapScriptMetadata[scriptID] = meta;
            ...
            ssKey >> masterPubKey;
            ssValue >> masterSeed;
            pwallet->setMasterSeed(masterPubKey, masterSeed, true);
...
bool CWalletDB::WriteHDMasterSeed(const CPubKey& pubkey, const CKeyingMaterial& masterSeed)
{
    nWalletDBUpdated++;
    return Write(std::make_pair(std::string("hdmasterseed"), pubkey), masterSeed);
}
