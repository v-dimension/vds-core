...
private:
    ...
    static const int DSEG_UPDATE_SECONDS        = 60;
    ...
    std::map<CAnonID, CMasternodeVerification> mWeAskedForVerification;

    // these maps are used for masternode recovery from MASTERNODE_NEW_START_REQUIRED state
    std::map<uint256, std::pair< int64_t, std::set<CAnonID> > > mMnbRecoveryRequests;
    std::map<uint256, std::vector<CMasternodeBroadcast> > mMnbRecoveryGoodReplies;
    std::list< std::pair<CAnonID, uint256> > listScheduledMnbRequestConnections;
...
public:
    ...
    void RandAskForMN(const COutPoint& outpoint);
    ...
    std::vector<CMasternode*> GetWinnerList(int nBlockHeight, bool fFilterSigTime, bool& bRetSuccess, bool bHeightCondition = true);
    std::vector<std::pair<int, CMasternode*> > GetMasternodeListLastPaid(int nBlockHeight, bool fFilterSigTime, bool& bRetSuccess, bool bHeightCondition = true);
    ...
    std::pair<CAnonID, std::set<uint256> > PopScheduledMnbRequestConnection();

    void ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman);
    void ProcessMessage(CMnAnonNode* pfrom, std::string& strCommand, CDataStream& vRecv, CMasternodeMan* connman);

    void DoFullVerificationStep(CConnman& connman);
    void CheckSameAddr();
    bool SendVerifyRequest(const CAnonID& addr, const std::vector<CMasternode*>& vSortedByAddr, CConnman& connman);
    void SendVerifyReply(CMnAnonNode* pnode, CMasternodeVerification& mnv, CMasternodeMan* anonconnman);
    void ProcessVerifyReply(CMnAnonNode* pnode, CMasternodeVerification& mnv, CMasternodeMan* anonconnman);
    void ProcessVerifyBroadcast(CMnAnonNode* pnode, const CMasternodeVerification& mnv, CMasternodeMan* anonconnman);
