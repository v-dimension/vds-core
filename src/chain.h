...
public:
    ...
    //! Number of clue transactions in this block.
    //! Note: in a potential headers-first mode, this number cannot be relied upon
    unsigned int nClueTx;

    unsigned int nChainClueTx;
    ...
    //! The debt to free heart
    int64_t nDebtTandia;

    //! Last block height of free heart paid
    int nHeightTandiaPaid;
...
public:
    ...
    inline void SerializationOp(Stream& s, Operation ser_action)
    {
        ...
        READWRITE(VARINT(nDebtTandia));
        READWRITE(VARINT(nHeightTandiaPaid));
        READWRITE(VARINT(nLastPaidTandia));
