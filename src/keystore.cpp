...
bool CBasicKeyStore::AddBTCWatchOnly(const CScript& dest)
{
    LOCK(cs_KeyStore);
    setBTCWatchOnly.insert(dest);
    return true;
}

bool CBasicKeyStore::RemoveBTCWatchOnly(const CScript& dest)
{
    LOCK(cs_KeyStore);
    setBTCWatchOnly.erase(dest);
    return true;
}

bool CBasicKeyStore::HaveBTCWatchOnly(const CScript& dest) const
{
    LOCK(cs_KeyStore);
    return setBTCWatchOnly.count(dest) > 0;
}

bool CBasicKeyStore::HaveBTCWatchOnly() const
{
    LOCK(cs_KeyStore);
    return (!setBTCWatchOnly.empty());
}
