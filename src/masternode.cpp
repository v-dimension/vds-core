...
CMasternode::CMasternode(CAnonID addr, COutPoint outpoint, CPubKey pubKeyCollateralAddress, CPubKey pubKeyMasternode, int nProtocolVersionIn) :
...
bool CMasternode::UpdateFromNewBroadcast(CMasternodeBroadcast& mnb, CConnman& connman)
{
    ...
    if (fMasterNode && pubKeyMasternode == activeMasternode.pubKeyMasternode) {
        ...
        if (nProtocolVersion == PROTOCOL_VERSION) {
            ...
            LogPrint("masternode", "CMasternode::UpdateFromNewBroadcast -- wrong PROTOCOL_VERSION, re-activate your MN: message nProtocolVersion=%d  PROTOCOL_VERSION=%d\n", nProtocolVersion, PROTOCOL_VERSION);
            return false;
        }
    }
    return true;
...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, int& nHeightRet)
{
    ...
    if (coin.out.nValue != MASTERNODE_COLLATERAL_COIN * COIN) {
...
bool CMasternode::IsInputAssociatedWithPubkey()
{
    ...
    if (GetTransaction(vin.prevout.hash, tx, Params().GetConsensus(), hash, true)) {
        ...
        if (out.nValue == MASTERNODE_COLLATERAL_COIN * COIN && out.scriptPubKey == payee) return true;
    }
...
bool CMasternode::IsValidNetAddr(CAnonID addrIn)
{
    return true;
...
string CAnonID::ToString() const
{
    return HexStr(begin(), end());
}

CId CAnonID::ToCid() const
{
    return CId((uint8_t*)begin());
}

void CAnonID::SetUint160(const CKeyID& keyid)
{
    *this = keyid;
}
