...
std::string EncodeHexMTx(const CMutableTransaction& tx, const int serialFlags)
{
    CDataStream ssTx(SER_NETWORK, PROTOCOL_VERSION | serialFlags);
    ssTx << tx;
    return HexStr(ssTx.begin(), ssTx.end());
...
UniValue TxShieldedSpendsToUniv(const CTransaction& tx)
{
...
UniValue TxShieldedOutputsToUniv(const CTransaction& tx)
{
    UniValue vdesc(UniValue::VARR);
...
void TxToUniv(const CTransaction& tx, const uint256& hashBlock, UniValue& entry)
{
    ...
    for (unsigned int i = 0; i < tx.vout.size(); i++) {
        ...
        out.push_back(Pair("dataHash", txout.dataHash.GetHex()));
    ...
    entry.push_back(Pair("valueBalance", FormatMoney(tx.valueBalance)));
    UniValue vspenddesc = TxShieldedSpendsToUniv(tx);
    entry.push_back(Pair("vShieldedSpend", vspenddesc));
    UniValue voutputdesc = TxShieldedOutputsToUniv(tx);
