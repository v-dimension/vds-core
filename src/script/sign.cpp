...
bool TransactionSignatureCreator::CreateSig(std::vector<unsigned char>& vchSig, const CKeyID& address, const CScript& scriptCode, SigVersion sigversion) const
{
    ...
    uint256 hash;
    try {
        hash = SignatureHash(scriptCode, *txTo, nIn, nHashType, sigversion);
...
{
    assert(tx.vin.size() > nIn);
//    tx.vin[nIn].scriptSig.empty();
//    if (!data.scriptWitness.IsNull()) {
//        tx.vin[nIn].scriptSig << ToByteVector(*(data.scriptWitness.stack.begin()));
//        tx.vin[nIn].scriptSig << ToByteVector(data.scriptWitness.stack.back());
//    }
