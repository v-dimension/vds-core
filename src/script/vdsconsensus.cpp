                                 ...
                                 unsigned int nIn, unsigned int flags, zcashconsensus_error* err)
{
    try {
        ...
        return VerifyScript(tx.vin[nIn].scriptSig, CScript(scriptPubKey, scriptPubKey + scriptPubKeyLen), &tx.vin[nIn].scriptWitness, flags, TransactionSignatureChecker(&mtx, nIn, txdata), NULL);
