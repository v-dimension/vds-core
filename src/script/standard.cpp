...
const std::vector<unsigned char> emptyuint160(20, '\0');

CScript feeAddress(CScript() << OP_DUP << OP_HASH160 << emptyuint160 << OP_EQUALVERIFY << OP_CHECKSIG);
...
bool ExtractContractAddress(const CScript& scriptPubKey, uint160& addressRet)
{
    ...
    if (whichType == TX_SCRIPTHASH) {
        addressRet = uint160(vSolutions[0]);
        return true;
    }
    // Multisig txns have more than one address...
    return false;
