...
/** Instant messaging settings port **/
static const unsigned int DEFAULT_INSTANT_MESSAGE_PORT = 9333;
...
public:
    ...
    // because it's used in many vds-specific places (masternode).
    ...
    CPubKey pubKey;
    bool SignMessage(const std::string strMessage, std::vector<unsigned char>& vchSigRet);
    bool VerifyMessage(const CPubKey pubkeyIn, const std::vector<unsigned char>& vchSig, const std::string strMessage);
private:
    struct ListenSocket {
        SOCKET socket;
        bool whitelisted;
...
public:
    ...
    std::vector<uint256> vAdToAnnonunce;
    ...
    std::set<uint256> vAdToGet;
    int64_t nGetAdTime;
    std::set<uint256> vAdGetSent;

    CNode(NodeId id, ServiceFlags nLocalServicesIn, int nMyStartingHeightIn, int nMyStartingLNHeightIn, SOCKET hSocketIn, const CAddress& addrIn, const std::string& addrNameIn = "", bool fInboundIn = false, bool fNetworkNodeIn = false);
    ~CNode();
