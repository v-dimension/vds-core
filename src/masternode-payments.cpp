...
*
*   Why is this needed?
*   - In vds some blocks are superblocks, which output much higher amounts of coins
...
bool IsBlockValueValid(const CBlock& block, int nBlockHeight, CAmount blockReward, std::string& strErrorRet)
{
    ...
    std::vector<uint8_t> vTypeExclude;
    vTypeExclude.push_back(CTxOut::VIB);
    bool isBlockRewardValueMet = (block.vtx[0]->GetValueOutWithExclude(vTypeExclude) <= blockReward);
    if (fDebug) LogPrint("masternode", "block.vtx[0]->GetValueOut() %lld <= blockReward %lld\n", block.vtx[0]->GetValueOut(), blockReward);
    ...
    CAmount nSuperblockMaxValue =  blockReward;
...
bool IsBlockPayeeValid(const CTransaction& txNew, int nBlockHeight, CAmount blockReward)
{
    if (!masternodeSync.IsSynced()) {
        ...
        if (fDebug) LogPrint("masternode", "IsBlockPayeeValid -- WARNING: Client not synced, skipping block payee checks\n");
        return true;
    }
...
void CMasternodePayments::FillBlockPayee(CMutableTransaction& txNew, int nBlockHeight, CAmount blockReward, CTxOut& txoutMasternodeRet)
{
    ...
    // ... and masternode
    txoutMasternodeRet = CTxOut(masternodePayment, CTxOut::MASTERNODE, payee);
...
//    sporkManager.IsSporkActive(SPORK_10_MASTERNODE_PAY_UPDATED_NODES)
...
void CMasternodePayments::ProcessMessage(CNode* pfrom, std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (strCommand == NetMsgType::MASTERNODEPAYMENTSYNC) { //Masternode Payments Request Sync
        ...
        if (netfulfilledman.HasFulfilledRequest(pfrom->addr, NetMsgType::MASTERNODEPAYMENTSYNC)) {
            ...
            LogPrint("masternode", "MASTERNODEPAYMENTSYNC -- peer already asked me for the list, peer=%d\n", pfrom->id);
        ...
        Sync(pfrom, connman);
        LogPrint("masternode", "MASTERNODEPAYMENTSYNC -- Sent Masternode payment votes to peer %d\n", pfrom->id);
...
bool CMasternodeBlockPayees::IsPayeeValid(const CScript& payeescript)
{
    LOCK(cs_vecPayees);
...
bool CMasternodePayments::IsPayeeValid(const CScript& payee, int nBlockHeight)
{
    if (!masternodeSync.IsSynced()) {
        //there is no budget data to use to check anything, let's just accept the longest chain
        if (fDebug) LogPrint("masternode", "IsBlockPayeeValid -- WARNING: Client not synced, skipping block payee checks\n");
    ...
    if (mapMasternodeBlocks.count(nBlockHeight)) {
        return mapMasternodeBlocks[nBlockHeight].IsPayeeValid(payee);
    }
}
...
bool CMasternodePayments::ProcessBlock(int nBlockHeight, CConnman& connman)
{
    ...
    if (!mnodeman.GetNextMasternodeInQueueForPayment(nBlockHeight, true, nCount, mnInfo)) {
        LogPrint("mnpayments", "CMasternodePayments::ProcessBlock -- ERROR: Failed to find masternode to pay\n");
        return false;
    }
    ...
    LogPrint("mnpayments", "CMasternodePayments::ProcessBlock -- vote: payee=%s, nBlockHeight=%d\n", EncodeDestination(address1), nBlockHeight);

    // SIGN MESSAGE TO NETWORK WITH OUR MASTERNODE KEYS

    LogPrint("mnpayments", "CMasternodePayments::ProcessBlock -- Signing vote\n");
    if (voteNew.Sign()) {
        LogPrint("mnpayments", "CMasternodePayments::ProcessBlock -- AddPaymentVote()\n");
