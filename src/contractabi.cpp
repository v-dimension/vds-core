...
std::string FunctionABI::errorMessage(std::vector<ParameterABI::ErrorType>& errors, bool in) const
{
    ...
    messages.push_back("ABI parsing error:");
    for (size_t i = 0; i < errors.size(); i++) {
        ParameterABI::ErrorType err = errors[i];
        if (err == ParameterABI::Ok) continue;
        const ParameterABI& param = params[i];
        std::string _type = param.type;
        std::string _name = param.name;

        switch (err) {
        case ParameterABI::UnsupportedABI:
            messages.push_back(strprintf("Unsupported type %s %s.", _type, _name));
            break;
        case ParameterABI::EncodingError:
            messages.push_back(strprintf("Error encoding parameter %s %s.", _type, _name));
            break;
        case ParameterABI::DecodingError:
            messages.push_back(strprintf("Error decoding parameter %s %s.", _type, _name));
            break;
        default:
            break;
        }
...
bool ParameterABI::getRegularExpession(const ParameterType& paramType, boost::regex& regEx)
{
    bool ret = false;
    switch (paramType.type()) {
    case ParameterType::abi_bytes: {
        if (paramType.isDynamic()) {
            regEx = boost::regex(paternBytes);
        } else {
            // Expression to check the number of bytes encoded in hex (1-32)
            regEx = boost::regex(strprintf(paternBytes32, paramType.totalBytes() * 2, paramType.totalBytes() * 2));
        }
        ret = true;
        break;
    }
    case ParameterType::abi_uint: {
        regEx = boost::regex(paternUint);
        ret = true;
        break;
    }
    case ParameterType::abi_int: {
        regEx = boost::regex(paternInt);
        ret = true;
        break;
    }
    case ParameterType::abi_address: {
        regEx = boost::regex(paternAddress);
        ret = true;
        break;
    }
    case ParameterType::abi_bool: {
        regEx = boost::regex(paternBool);
        ret = true;
        break;
    }
    default: {
